...
class RankingActionForm extends FormBase {
   ...
   */ 
  function MarkCalculations($tid) {
      try {
        // Get users with role 'candidato_enviado' who have research_data for this field
        ...
        foreach ($user_ids as $uid) {
          ...
          if (isset($marks[0])) {$interest_node->set('field_marks_eva1', $marks[0]);};
          if (isset($marks[1])) {$interest_node->set('field_marks_eva2', $marks[1]);};
          if (isset($marks[2])) {$interest_node->set('field_marks_eva3', $marks[2]);};
          if (isset($difference)) {$interest_node->set('field_difference', $difference);};
          $interest_node->save();
          $candidates_rank[] = $uid;
        }
  ...
  function RankingCalculations($tid) {
    try {
      // Get users with role 'candidato_enviado' who have research_data for this field
      $query = \Drupal::entityQuery('user')
        ->accessCheck(TRUE)
