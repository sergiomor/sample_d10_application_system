...
/**
 * Service for managing research application rankings.
 *
 * Provides functionality for:
 * - Calculating candidate rankings
 * - Processing evaluation scores
 * - Managing research line associations
...
/**
 * Retrieves ranking data for candidates in a research line.
 *
 * @param int $tid
 ...
 */
  public function getRankingsData($tid) {
    ...
    usort($rankings, function ($a, $b) {
      ...
      // Push non-numeric values to the end.
          : PHP_INT_MAX;
      ...
      if ($a_val == $b_val) {
        ...
        // Higher marks first.
          return $b_mark <=> $a_mark;
      }

      // Lower positions first (1st place, 2nd place, etc.)
      return $a_val <=> $b_val;
    });
    ...
    if (!$third_evaluator) {
      ...
      // Break the reference to the last element.
      unset($rank);
    ...
    foreach ($rankings as $key => $value) {
      // Deep clone.
      $rankings_copy[$key] = unserialize(serialize($value));
    }
    ...
    if ($fid) {
      ...
      $diff_row = [
        ...
        'marks_eva1' => ['data' => $stats['marks_eva1']['diff_max_min'] ?? '-', 'class' => ['summary-row']],
        'marks_eva2' => ['data' => $stats['marks_eva2']['diff_max_min'] ?? '-', 'class' => ['summary-row']],
      ];

      if ($include_third_evaluator) {
        $diff_row['marks_eva3'] = ['data' => $stats['marks_eva3']['diff_max_min'] ?? '-', 'class' => ['summary-row']];
      }

      $diff_row['average'] = ['data' => $stats['average']['diff_max_min'] ?? '-', 'class' => ['summary-row']];
      $rows[] = $diff_row;
      ...
      $avg_row = [
        ...
        'marks_eva1' => ['data' => $stats['marks_eva1']['average'] ?? '-', 'class' => ['summary-row']],
        'marks_eva2' => ['data' => $stats['marks_eva2']['average'] ?? '-', 'class' => ['summary-row']],
      ];

      if ($include_third_evaluator) {
        $avg_row['marks_eva3'] = ['data' => $stats['marks_eva3']['average'] ?? '-', 'class' => ['summary-row']];
      }

      $avg_row['average'] = ['data' => $stats['average']['average'] ?? '-', 'class' => ['summary-row']];
      $rows[] = $avg_row;
      ...
      $dev_row = [
        ...
        'marks_eva1' => ['data' => $stats['marks_eva1']['deviation'] ?? '-', 'class' => ['summary-row']],
        'marks_eva2' => ['data' => $stats['marks_eva2']['deviation'] ?? '-', 'class' => ['summary-row']],
      ];

      if ($include_third_evaluator) {
        $dev_row['marks_eva3'] = ['data' => $stats['marks_eva3']['deviation'] ?? '-', 'class' => ['summary-row']];
      }

      $dev_row['average'] = ['data' => $stats['average']['deviation'] ?? '-', 'class' => ['summary-row']];
      $rows[] = $dev_row;
    }
