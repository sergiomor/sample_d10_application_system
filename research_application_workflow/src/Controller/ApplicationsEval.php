...
class ApplicationsEval extends ControllerBase {
  ...
  public function getRankings($user) {
    ...
    if ($user_entity->hasField('field_research_lines')) {
      ...
      // Check if there are research lines and if the 
      // taxonomy term type has the field_ranking_pdf field.
      $has_ranking_field = FALSE;

      // Check if at least one research line 
      // taxonomy term has the field_ranking_pdf.
      if (!empty($research_lines)) {
        ...
        if ($sample_tid) {
          $sample_term = Term::load($sample_tid);
          if ($sample_term && $sample_term->hasField('field_ranking_pdf')) {
            ...
            // Only set has_ranking_field to true if
            // the field exists and has a valid file.
            if ($file_exists) {
      ...
      // Check if all candidates have evaluation
      // files submitted by this evaluator.
      ...
      if (!empty($c_users)) {
        foreach ($c_users as $c_user) {
          ...
          // Check if this candidate has a submitted
          // evaluation file from this evaluator.
      ...
      // 2. All candidates have evaluation files 
      // submitted by this evaluator.
            ...
            if ($term) {
              ...
              if ($term->hasField('field_ranking_pdf') && !$term->get('field_ranking_pdf')->isEmpty()) {
                ...
                if ($ranking_pdf_fid) {
                  ...
                  // Check if the evaluator has already 
                  // uploaded an approval file.
