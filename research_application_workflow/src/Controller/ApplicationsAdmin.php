...
class ApplicationsAdmin extends ControllerBase {
  ...
  /**
   * Creates a dashboard for application management
   */
  ...
  public function getFilteredResults($field_of_research_filter = '') {
    ...
    foreach ($c_users as $c_user) {
      ...
      // Skip this user if a field of research filter 
      // is applied and doesn't match.
      if ($field_of_research_filter && !empty($field_of_research_filter)) {

        // Check if the user has research data and the 
        // field_choose_field matches the filter.
